...
const P_layout:FC = () => {
  ...
  return (
    <div className="w-full h-screen snap overflow-y-auto scrolling-touch">
        ...
        >
            <nav className="fixed top-0 left-0 z-20 w-full flex flex-wrap items-center justify-between py-4  text-gray-500 hover:text-gray-700 focus:text-gray-700 navbar navbar-expand-lg navbar-light w-screen">
                    <div className="container-fluid w-full flex flex-wrap items-center justify-center px-12">
                        <ul className="hidden md:flex md:flex-row">
                            {siteConfig.portfotioNav.map((menu, index) => (
                                <li key={index}>
                                    <a
                                        className="block rounded-lg px-8 py-2 text-slate-700 text-lg font-medium hover:bg-slate-100 hover:text-slate-900"
                                        href={`#${menu}`}
